...
public class RouteInfoManager {
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // rocketMQ是由brokerId来确定主从的， brokerId为0的broker为master-broker
                //Switch slave to master: first remove <1, IP:PORT> in namesrv, then add <0, IP:PORT>
                //The same IP:PORT must only have one record in brokerAddrTable
                Iterator<Entry<Long, String>> it = brokerAddrsMap.entrySet().iterator();
                while (it.hasNext()) {
                    Entry<Long, String> item = it.next();
                    // 删除已有数据中 id不同，但地址相同的数据
                ...
                // topic配置不为空， 且当前broker为master-broker， 新增或修改QueueData信息
                ...
                // slave-broker 关联master-broker信息
    ...
    public boolean isBrokerTopicConfigChanged(final String brokerAddr, final DataVersion dataVersion) {
        // 从 brokerLiveTable 中取当前地址broker的数据版本
    ...
    private void createAndUpdateQueueData(final String brokerName, final TopicConfig topicConfig) {
        ...
        // 从 topicQueueTable 中根据topicName获取可能已存在数据
        ...
        if (null == queueDataList) {
            ...
            while (it.hasNext()) {
                ...
                if (qd.getBrokerName().equals(brokerName)) {
                    // 根据brokerName和topicName获取之前可能存在的记录，判断是否新添加
